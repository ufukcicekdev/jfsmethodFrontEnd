/**
 * HumanBodySVG — front-view segmented human body silhouette.
 *
 * SVG path data sourced from volcanioo/Human-Body-Rendering-HTML (MIT licence)
 * https://github.com/volcanioo/Human-Body-Rendering-HTML
 *
 * Each body part is its own <svg> element positioned with absolute layout
 * inside a fixed-size container (110 × 330 px by default).
 *
 * Usage:
 *   <HumanBodySVG />
 *   <HumanBodySVG width={130} fillColor="#a0c4ff" strokeColor="#2563eb" />
 *   <HumanBodySVG highlightedParts={["chest", "stomach"]} highlightColor="#f97316" />
 */
...
interface HumanBodySVGProps {
  /** Rendered width in px (height scales proportionally at ~3× width) */
  width?: number;
  /** Default fill colour for all body parts */
  fillColor?: string;
  /** Stroke colour (outline) */
  strokeColor?: string;
  /** Which body parts to highlight */
  highlightedParts?: BodyPart[];
  /** Highlight fill colour */
  highlightColor?: string;
  /** Called with the body-part name when a region is clicked */
  onPartClick?: (part: BodyPart) => void;
  className?: string;
}

/* ---------------------------------------------------------------------------
 * Raw path data — each entry carries its own natural viewBox so the paths
 * render correctly when scaled.
 * ---------------------------------------------------------------------------*/
const PARTS: Record<
  BodyPart,
  {
    viewBox: string;
    d: string;
    /** position as fraction of container [left%, top%, width%, height%] */
...
> = {
  ...
  rightLeg: {
    ...
    d: "m 26.664979,139.7913 0.663,5.99 -4.647,5.77 -1.55,9.1 -3.1,1.33 -2.655,-13.755 -1.77,-4.88 1.55,-3.107 z m -20.5820002,0.444 3.3200005,9.318 7.0799997,13.755 -1.77,12.647 -5.0899997,-14.2 -4.2000005,-7.987 z m 3.7620005,29.73 1.5499997,4.88 3.32,20.633 0.442,27.51 -4.648,26.847 0.22,-34.39 -4.8670002,-13.754 -0.67,-15.087 z m 10.6229997,12.424 -1.107,41.267 -8.852,33.28 9.627,-4.55 16.046,-57.8 -5.533,-36.384 z m -13.9460002,74.991 c -5.157661,19.45233 -2.5788305,9.72616 0,0 z M 30.177979,4.225305 l 0,22.476 -15.713,31.072 -9.9230002,30.850005 1.033,-21.375005 z m -25.4930002,30.249 -0.118,-0.15 0.793,-2.023 16.5450002,-18.16 1.24,-0.44 -10.98,28.377 z m 6.2550002,10.764 -6.8120002,17.6 -2.274,-21.595 1.344,-3.43 z m 26.47,17.82 -0.827,25.342005 -12.816,35.25599 3.927,10.136 12.61,-44.50999 z m -24.565,12.783005 -0.346,0.825 -6.4700002,15.48 4.1780002,38.34199 6.594,-3.527 -5.715,-35.69999 z m 1.671,-126.452995 -0.826,21.375 7.03,42.308 3.306,-34.155 z m -2.066,27.325 -14.677,32.392 -3.308,26.000005 6.2,36.57999 13.436,-37.23999 0.827,-38.340005 z",
    pos: [50, 48, 32, 45],
  },
  leftHand: {
...
export default function HumanBodySVG({
  ...
  return (
    ...
    >
      {(Object.entries(PARTS) as [BodyPart, (typeof PARTS)[BodyPart]][]).map(
        ([part, { viewBox, d, pos }]) => {
          const [leftPct, topPct, widthPct, heightPct] = pos;
          const isHighlighted = highlightedParts.includes(part);
          const fill = isHighlighted ? highlightColor : fillColor;
          const stroke = isHighlighted ? highlightColor : strokeColor;
          ...
          return (
            <svg
              ...
              aria-label={part}
              role={onPartClick ? "button" : undefined}
            >
